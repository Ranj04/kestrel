...
export function WhyDrawer({ alert, onClose }: { alert: Alert; onClose: () => void }) {
  return (
    <div className="slide-in absolute inset-0 z-20 flex flex-col overflow-hidden border-r border-line bg-paper-raised">
      <div className="flex items-start justify-between border-b border-line px-5 py-3">
        ...
        {/* the import's Close: an underlined mono label, not a boxed button */}
        <button
          onClick={onClose}
          className="border-b border-ink pb-0.5 font-mono text-xs uppercase tracking-[0.16em] text-ink"
        >
          Close
        </button>
      </div>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-5 pb-4">
        {/* VERBATIM from data/cpic/index.json — text-lg resolves to --step-1, the same
            step the recommendation carries on the card. The drawer is the SAME
            sentence with provenance, so it repeats at the same size. No vermilion
            rule here: --accent is the critical alert and a broken chain, only. */}
        <blockquote className="mt-3 max-w-[60ch] text-pretty font-display text-lg leading-snug">
          &ldquo;{alert.recommendation}&rdquo;
        </blockquote>

        <p className="mt-3 flex flex-wrap items-center gap-3 font-mono text-xs">
          {alert.cpicLevelA && (
            <span className="border border-ink px-2 py-0.5 font-semibold uppercase tracking-[0.16em]">
              CPIC LEVEL A
            </span>
          )}
        ...
        {alert.fdaLabeled && (
          <div className="mt-3 border border-dashed border-line px-3 py-2">
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="mt-1.5 block break-all font-mono text-xs text-ink-soft underline decoration-line underline-offset-2 hover:text-ink"
            >
              ...
              {alert.citations.map((c) => (
                <li key={c.pmid}>
                  <a
                    ...
                    className="underline decoration-line underline-offset-2 hover:decoration-ink"
                  >
                    <span className="font-mono text-xs">PMID {c.pmid}</span> · {c.title} ·{" "}
                    {c.year}
                  </a>
                </li>
              ))}
      ...
      {/* provenance pinned to the drawer's floor, outside the scroll region, per
          the import — the last thing under the evidence is where it came from. */}
      <div className="border-t border-line px-5 py-2">
        <p className="font-mono text-xs uppercase tracking-[0.18em] text-ink-soft">source record</p>
        {/* the exact CPIC API row this alert came from — the value itself, never rebuilt */}
        <a
          href={alert.sourceUrl}
          target="_blank"
          rel="noreferrer"
          className="mt-0.5 block break-all font-mono text-xs text-ink-soft underline decoration-line underline-offset-2 hover:text-ink"
        >
          {alert.sourceUrl}
        </a>
      </div>
